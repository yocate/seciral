...
def update_author(conn):
    cursor = conn.cursor()
    cursor.execute("UPDATE graph_nodes SET author = '神保良弘' WHERE author = 'System' OR author IS NULL OR author = ''")
    nodes_updated = cursor.rowcount
    cursor.execute("UPDATE graph_edges SET author = '神保良弘' WHERE author = 'System' OR author IS NULL OR author = ''")
    edges_updated = cursor.rowcount
    conn.commit()
    print(f"Author updated: {nodes_updated} nodes, {edges_updated} edges")
...
"""
    ...
    try:
        ...
        print(f"Found {len(merges)} potential merge groups.")
        ...
        for m in merges:
            ...
            for m_id in merge_ids:
                ...
                # Update edges
                cursor.execute("UPDATE graph_edges SET source = ? WHERE source = ?", (keep_id, m_id))
                cursor.execute("UPDATE graph_edges SET target = ? WHERE target = ?", (keep_id, m_id))
                
                # Delete merged node
                cursor.execute("DELETE FROM graph_nodes WHERE id = ?", (m_id,))
                print(f"Merged {m_id} into {keep_id}")
                
        conn.commit()
    except Exception as e:
        print(f"Error parsing or applying JSON: {e}")
